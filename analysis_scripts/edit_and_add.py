...
class AttackSample:
    def __init__(self, context, context_sents, context_sents_offsets, question, answers, id, title):
        self.context = context
        self.context_sents = context_sents
        self.context_sents_offsets = context_sents_offsets
...
class QAAttacker:
    ...
    def evaluate_edit_importance(self, samples):
        ...
        for i in range(len(indices)):
            ...
            samples[original_index]["pred_start_position"] = best_start_positions[indices[i][1]].item()
            samples[original_index]["pred_end_position"] = best_end_positions[indices[i][1]].item()
    ...
    def _sample_nouns_same_level(self, noun):
        possible_candidates = []
        ...
        for synset in synsets:
            if synset.lemma_names()[0] == noun and synset.lexname().split(".")[0] == "noun":
        ...
        for synset in same_synsets:
            ...
            for hyper in hypernyms:
                ...
                hypos = [h.lemma_names()[0].replace("_", " ") for h in hypos if h.lemma_names()[0] != noun]
    ...
    def edit_distractors(self, sample, ent_dict, ents=True):
        ...
        if ents:
            ...
            for i, target_noun in enumerate(sample["add_candidates"]["nouns"]):
                if edit_cnt > self.max_edit_num_in_add:
                    break
                sampled_nouns = self._sample_nouns_same_level(target_noun.lemma_)
        ...
        for sample in samples:
            ...
            if sample.__contains__("contexts_with_distractor") and len(sample["contexts_with_distractor"]) > 0:
                for i, context in enumerate(sample["contexts_with_distractor"][:self.remain_size_per_sample]):
                    ...
                    adv_sample_index += 1
                    sample_cnt += 1
            elif len(sample["edited_gold_sents"]) > 0:
...
def main():
    ...
    qa_attacker = QAAttacker(args, model_evaluator, has_answer_evaluator, ppl_evaluator, use_evaluator)
