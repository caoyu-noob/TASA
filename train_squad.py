...
def post_processing_function(examples, features, predictions, args, answer_column_name, stage="eval"):
    ...
    return EvalPrediction(predictions=formatted_predictions, label_ids=references), predictions, all_nbest_json
...
def main():
    args = parse_args()
    if args.do_predict:
        logger = config_logger(os.path.join(args.output_dir, 'train.log'))
    else:
        logger = config_logger(os.path.join(args.output_dir, 'predict.log'))
    ...
    column_names = raw_datasets["train"].column_names if raw_datasets.__contains__("train") else \
        raw_datasets["test"].column_names
    ...
    if not args.do_predict:
        ...
        best_metric, best_acc, best_f1 = 0, 0, 0
        for epoch in tqdm(range(args.num_train_epochs), desc="Training epoch"):
            model.train()
            completed_steps = 0
            progress_bar = tqdm(range(num_update_steps_per_epoch), disable=not accelerator.is_local_main_process)
            avg_loss, cur_loss = 0, 0
            for step, batch in enumerate(train_dataloader):
                ...
                if step % args.gradient_accumulation_steps == 0 or step == len(train_dataloader) - 1:
                    ...
                    completed_steps += 1
                    avg_loss = (avg_loss * (completed_steps - 1) + cur_loss) / completed_steps
                    cur_loss = 0
